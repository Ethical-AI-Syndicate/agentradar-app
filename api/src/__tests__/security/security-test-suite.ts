    ...
    describe("JWT Security", () => {
      ...
      it("should reject tampered tokens", async () => {
        ...
        const tamperedTokens = [
          ...
          validToken.slice(0, validToken.indexOf(".")) +
            ".xxxxx." +
            validToken.split(".")[2], // Tamper payload
        ];
      ...
      it("should accept requests from trusted origins", async () => {
        ...
        for (const origin of trustedOrigins) {
          const response = await request(this.app)
            .post(endpoint)
            .set("Authorization", `Bearer ${token}`)
            .set("Origin", origin || "http://localhost:3000")
    ...
    describe("Rate Limiting", () => {
      it("should rate limit requests per IP", async () => {
        ...
        const requests = Array(maxRequests + 10)
          .fill(null)
          .map(() => request(this.app).get(endpoint));
      ...
      it("should have different rate limits for different endpoints", async () => {
        ...
        const regularLimit = parseInt(
          regularResponse.headers["x-ratelimit-limit"],
        );
  ...
  async testAuthorization(
    ...
    describe("Authorization", () => {
      it("should require proper role for access", async () => {
        const user = await testDb.createUser({ role: UserRole.USER });
      ...
      it("should allow access with correct role", async () => {
        ...
        const token = await jwt.createUserToken(
          admin.id,
          admin.email,
        ...
        describe(`${rule.field} validation`, () => {
          ...
          if (rule.minLength) {
            it(`should validate ${rule.field} minimum length`, async () => {
              ...
              const payload = {
                ...rule.validPayload,
                [rule.field]: "x".repeat((rule.minLength || 1) - 1),
              };
          ...
          if (rule.maxLength) {
            it(`should validate ${rule.field} maximum length`, async () => {
              ...
              const payload = {
                ...rule.validPayload,
                [rule.field]: "x".repeat((rule.maxLength || 100) + 1),
              };
