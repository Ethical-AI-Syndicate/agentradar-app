      ...
      for (const endpoint of endpoints) {
        it(`should respond within ${this.performanceThresholds.apiResponseTime}ms for ${endpoint}`, async () => {
          ...
          const { result: response, duration } =
            await perfHelper.measureExecutionTime(async () => {
          ...
          expect(duration).toBeLessThan(
            this.performanceThresholds.apiResponseTime,
          );
        ...
        it(`should maintain consistent response times under load for ${endpoint}`, async () => {
          ...
          const responses = await perfHelper.createConcurrentRequests(
            async () => {
          ...
          const sortedDurations = responses
            .map((r) => r.duration)
            .sort((a, b) => a - b);
          ...
          expect(p95ResponseTime).toBeLessThan(
            this.performanceThresholds.apiResponseTime * 2,
          );

          // Verify success rate
          const successfulRequests = responses.filter(
            (r) => r.response.status < 400,
          );
  ...
  async testDatabasePerformance(): Promise<void> {
    describe("Database Performance", () => {
      it("should execute simple queries within threshold", async () => {
        ...
        expect(duration).toBeLessThan(
          this.performanceThresholds.databaseQueryTime,
        );
      });
      ...
      it("should handle complex alert queries efficiently", async () => {
        ...
        expect(duration).toBeLessThan(
          this.performanceThresholds.databaseQueryTime * 5,
        ); // Allow 5x for complex queries
      ...
      it("should handle database transactions efficiently", async () => {
        ...
        expect(duration).toBeLessThan(
          this.performanceThresholds.databaseQueryTime * 10,
        );
      ...
      it("should maintain connection pool efficiency", async () => {
        const connectionPromises = Array(20)
          .fill(null)
          .map(async () => {
            const { duration } = await perfHelper.measureExecutionTime(
              async () => {
  ...
  async testLoadCapacity(endpoint: string): Promise<void> {
    describe("Load Testing", () => {
      it("should handle concurrent users within threshold", async () => {
        const concurrentUsers = Math.min(
          this.performanceThresholds.concurrentUsers,
          100,
        ...
        expect(requestsPerSecond).toBeGreaterThan(
          this.performanceThresholds.requestsPerSecond / 2,
        );
        ...
        expect(successRate).toBeGreaterThan(
          1 - this.performanceThresholds.errorRate,
        );
      ...
      it("should maintain performance under sustained load", async () => {
        ...
        while (performance.now() - startTime < duration) {
          const { result, duration: reqDuration } =
            await perfHelper.measureExecutionTime(async () => {
              return request(this.app)
                .get(endpoint)
                .set("Authorization", `Bearer ${token}`);
        ...
        const avgFirstHalf =
          firstHalf.reduce((sum, r) => sum + r.duration, 0) / firstHalf.length;
        const avgSecondHalf =
          secondHalf.reduce((sum, r) => sum + r.duration, 0) /
          secondHalf.length;
  ...
  async testMemoryUsage(endpoint: string): Promise<void> {
    describe("Memory Usage", () => {
      ...
      it("should handle large datasets without memory leaks", async () => {
        const { result, memoryDelta } = await perfHelper.measureMemoryUsage(
          async () => {
    ...
    describe("Alert Matching Performance", () => {
      it("should handle large scale alert matching efficiently", async () => {
        ...
        const alerts = await Promise.all(
          dataGenerator
            .generateBulkAlerts(1000)
            .map((alert) => testDb.createAlert(alert)),
        );

        const { duration } = await perfHelper.measureExecutionTime(async () => {
          // Simulate alert matching for all alerts
          const matchPromises = alerts.slice(0, 100).map(
            (
              alert, // Limit for testing
            ) =>
  ...
  async testStressLimits(endpoint: string): Promise<void> {
    describe("Stress Testing", () => {
      it("should gracefully handle extreme load", async () => {
        ...
        const requests = Array(extremeLoad)
          .fill(null)
          .map(async () => {
            try {
  ...
  async testPerformanceRegression(
    ...
    describe("Performance Regression", () => {
      it("should not regress from baseline performance", async () => {
        ...
        if (baselineMetrics) {
          expect(avgDuration).toBeLessThan(
            baselineMetrics.avgResponseTime * 1.2,
          ); // 20% tolerance
          expect(p95Duration).toBeLessThan(
            baselineMetrics.p95ResponseTime * 1.2,
          );
        } else {
