...
expect.extend({
  toBeValidJWT(received: string) {
    ...
    if (jwtParts.length !== 3) {
      return {
        pass: false,
        message: () => `Expected JWT with 3 parts, but received ${jwtParts.length} parts`
      };
    }
    ...
    try {
      ...
      return {
        pass: false,
        message: () => `Invalid JWT format: ${error.message}`
      };
    }
...
expect.extend({
  toMatchUserSchema(received: any) {
    const requiredFields = ['id', 'email', 'firstName', 'lastName', 'subscriptionTier'];
    ...
    const presentForbiddenFields = forbiddenFields.filter(field => field in received);
    if (presentForbiddenFields.length > 0) {
      return {
        pass: false,
        message: () => `Contains forbidden fields: ${presentForbiddenFields.join(', ')}`
      };
    }

    // Validate email format
    if (!received.email || typeof received.email !== 'string' || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(received.email)) {
      return {
        pass: false,
    ...
    const validTiers = ['FREE', 'SOLO_AGENT', 'PROFESSIONAL', 'TEAM_ENTERPRISE', 'WHITE_LABEL'];
    if (!validTiers.includes(received.subscriptionTier)) {
      return {
        pass: false,
        message: () => `Invalid subscription tier: ${received.subscriptionTier}`
      };
    }
...
expect.extend({
  toMatchAlertSchema(received: any) {
    const requiredFields = ['id', 'title', 'description', 'address', 'city', 'province', 'alertType', 'status', 'priority', 'opportunityScore'];
    ...
    // Validate alert type
    const validAlertTypes = ['POWER_OF_SALE', 'ESTATE_SALE', 'DEVELOPMENT_APPLICATION', 'MUNICIPAL_PERMIT', 'PROBATE_FILING', 'TAX_SALE'];
    ...
    if (typeof received.opportunityScore !== 'number' || received.opportunityScore < 0 || received.opportunityScore > 100) {
      return {
        pass: false,
        message: () => `Invalid opportunity score: ${received.opportunityScore} (must be 0-100)`
      };
    }
...
expect.extend({
  toHaveValidPagination(received: any) {
    ...
    if (missingPaginationFields.length > 0) {
      return {
        pass: false,
        message: () => `Missing pagination fields: ${missingPaginationFields.join(', ')}`
      };
    }
...
expect.extend({
  toHaveValidCoordinates(received: any) {
    ...
    if (typeof latitude !== 'number' || typeof longitude !== 'number') {
      return {
        pass: false,
        message: () => `Coordinates must be numbers. Received lat: ${typeof latitude}, lng: ${typeof longitude}`
      };
    }
...
expect.extend({
  toBeValidMonetaryAmount(received: any) {
    ...
    if (!Number.isInteger(received)) {
      return {
        pass: false,
        message: () => `Monetary amount should be in cents (integer): ${received}`
      };
    }
...
expect.extend({
  toMatchApiErrorFormat(received: any) {
    ...
    const missingRequired = requiredFields.filter(field => !(field in received));
    if (missingRequired.length > 0) {
      return {
        pass: false,
        message: () => `Missing required error fields: ${missingRequired.join(', ')}`
      };
    }
...
expect.extend({
  toHaveSecureHeaders(received: any) {
    ...
    const missingHeaders = requiredHeaders.filter(header => !received.get(header));
