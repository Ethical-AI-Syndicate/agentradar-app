...
async function setupTestDatabase(): Promise<void> {
  ...
  const testDbName = `agentradar_test_${timestamp}`;
  ...
  try {
    ...
    await prisma.$executeRaw`CREATE DATABASE IF NOT EXISTS ${testDbName}`;
    ...
    // Apply critical indexes from Phase 4
    const indexMigrationPath = path.join(
      __dirname, 
      '../../../database/migrations/001_add_critical_indexes.sql'
    );
    
    if (fs.existsSync(indexMigrationPath)) {
      const indexSql = fs.readFileSync(indexMigrationPath, 'utf8');
      
      // Execute index creation (remove CONCURRENTLY for test environment)
      const testIndexSql = indexSql.replace(/CONCURRENTLY/g, '');
      await prisma.$executeRawUnsafe(testIndexSql);
    }
    ...
    console.log('✅ Test database configured:', testDbName);
...
async function verifyTestEnvironment(): Promise<void> {
  ...
  const testUserCount = await global.__GLOBAL_TEST_CONFIG__.database.client.user.count();
  if (testUserCount === 0) {
...
async function seedTestData(prisma: PrismaClient): Promise<void> {
  ...
  const alertTypes = ['POWER_OF_SALE', 'ESTATE_SALE', 'DEVELOPMENT_APPLICATION'];
