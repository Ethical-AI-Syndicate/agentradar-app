  ...
  try {
    ...
    for (const table of tablesToClean) {
      try {
        ...
        console.log(`  ⚠️  Warning: Could not clean table ${table}:`, error.message);
      }
    }
...
async function cleanupRedis(): Promise<void> {
  try {
    ...
    console.log('  ⚠️  Redis cleanup failed (non-critical):', error.message);
  }
}
...
async function cleanupTempFiles(): Promise<void> {
  try {
    ...
    try {
      ...
      const testLogFiles = logFiles.filter(file => 
    ...
    console.log('  ⚠️  Temporary files cleanup failed (non-critical):', error.message);
  }
}
...
export async function cleanupTestScenario(scenarioName: string): Promise<void> {
  ...
  try {
    ...
    switch (scenarioName) {
      ...
      default:
        console.log(`  ⚠️  No specific cleanup defined for scenario: ${scenarioName}`);
    }
...
export function reportMemoryUsage(): void {
  ...
  if (heapUsedMB > 500) {
    console.log('⚠️  High memory usage detected. Consider investigating memory leaks.');
  }
}
