...
export class TestDatabaseHelper {
  ...
  async createAlertPreference(
    ...
    const defaultPreferences = {
      ...
      maxDistanceKm: 50,
      alertTypes: [AlertType.POWER_OF_SALE, AlertType.ESTATE_SALE],
...
export class ApiTestHelper {
  ...
  async testExpiredToken(
    ...
    const expiredToken = await JWTHelper.createExpiredToken({
      userId: "test",
  ...
  async testRateLimit(
    ...
    const requests = Array(maxRequests + 1)
      .fill(null)
      .map(() =>
        request(this.app).get(endpoint).set("Authorization", `Bearer ${token}`),
      );
