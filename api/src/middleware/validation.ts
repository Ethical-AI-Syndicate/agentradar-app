...
export const alertValidationSchema = Joi.object({
  title: Joi.string()
    ...
    .messages({
      "any.invalid":
        "Title contains invalid characters or potential security threats",
  ...
  estimatedValue: Joi.number().min(0).max(999999999).optional().messages({
  ...
  opportunityScore: Joi.number().min(0).max(100).optional(),
});
...
export const preferencesValidationSchema = Joi.object({
  ...
  minOpportunityScore: Joi.number().min(0).max(100).optional(),

  minEstimatedValue: Joi.number().min(0).max(999999999).optional(),

  maxEstimatedValue: Joi.number().min(0).max(999999999).optional(),
  ...
  dailyAlertLimit: Joi.number().min(1).max(100).optional(),
