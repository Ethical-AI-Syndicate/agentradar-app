 ...
 */
export const realEstateCompliance = (
  req: ComplianceRequest,
  res: Response,
  next: NextFunction,
): void | Response => {
 ...
 */
export const dataRetentionCompliance = async (
  req: ComplianceRequest,
  res: Response,
  next: NextFunction,
): Promise<void> => {
  try {
    ...
    if (
      req.compliance?.dataAccessed &&
      req.compliance.dataAccessed.length > 0
    ) {
      await scheduleRetentionCheck(
        req.compliance.dataAccessed,
        req.compliance.retentionPolicy || "2555",
      );
    }
...
function checkLicenseRequirement(req: Request): boolean {
  ...
  return (
    licensedPaths.some((path) => req.path.includes(path)) &&
...
async function scheduleRetentionCheck(
  dataAccessed: string[],
