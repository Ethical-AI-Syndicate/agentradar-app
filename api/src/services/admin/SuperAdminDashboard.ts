...
export class SuperAdminDashboard {
    ...
    try {
      ...
      const metrics: BusinessHealthMetrics = {
        ...
        arr: {
          ...
          projection: this.calculateARRProjection(
            mrrData.value,
            mrrData.growthRate,
          ),
        },
  ...
  async getQuickActions(): Promise<QuickAction[]> {
    return [
      // Emergency controls
      {
        ...
        description:
          "Temporarily halt all AI API calls to prevent cost overrun",
  ...
  async executeQuickAction(
    ...
    try {
      logger.info(
        `Executing quick action: ${actionId} by admin: ${adminUserId}`,
      );
  ...
  private async calculateChurnMetrics() {
    ...
    const churnRate =
      totalCustomers > 0
        ? (analytics.cancelledSubscriptions / totalCustomers) * 100
        : 0;
  ...
  private async calculateLTVMetrics() {
    ...
    const users = await prisma.user.groupBy({
      by: ["subscriptionTier"],
      _count: true,
      // _avg removed since we don't have numeric fields to average
    });
  ...
  private calculateARRProjection(
    currentMRR: number,
    growthRate: number,
  ): number {
  ...
  private async getOpenTicketsMetrics() {
    ...
    tickets.forEach((ticket) => {
      byPriority[ticket.priority] =
        (byPriority[ticket.priority] || 0) + ticket._count;
      byCategory[ticket.category] =
        (byCategory[ticket.category] || 0) + ticket._count;
    });
  ...
  private async getResolutionTimeMetrics() {
    ...
    const average =
      resolutionTimes.length > 0
        ? resolutionTimes.reduce((sum, rt) => sum + rt.time, 0) /
    ...
    ["URGENT", "HIGH", "MEDIUM", "LOW"].forEach((priority) => {
      const priorityTimes = resolutionTimes.filter(
        (rt) => rt.priority === priority,
      );
      byPriority[priority] =
        priorityTimes.length > 0
          ? priorityTimes.reduce((sum, rt) => sum + rt.time, 0) /
            priorityTimes.length /
            (1000 * 60 * 60)
          : 0;
    });
  ...
  private async getAgentPerformanceMetrics() {
    ...
    const avgTicketsPerAgent =
      agentStats.length > 0 ? totalTickets / agentStats.length : 0;
  ...
  private async getEscalationRateMetrics() {
    ...
    const escalatedTickets = await prisma.supportTicket.count({
      where: {
        ...
        priority: "URGENT", // Using priority as proxy for escalation
      },
    });

    const percentage =
      totalTickets > 0 ? (escalatedTickets / totalTickets) * 100 : 0;
    ...
    if (adminUserId) {
      await prisma.adminAction.create({
        data: {
          ...
          metadata: { reason: "Emergency pause via dashboard" },
          targetType: "SYSTEM",
          description: "Emergency pause AI processing via dashboard",
        },
      });
    }
    ...
    if (adminUserId) {
      await prisma.adminAction.create({
        data: {
          ...
          metadata: { reason: "Emergency disable via dashboard" },
          targetType: "SYSTEM",
          description: "Emergency disable signups via dashboard",
        },
      });
    }
    ...
    if (adminUserId) {
      await prisma.adminAction.create({
        data: {
          ...
          metadata: { reason: "Emergency maintenance via dashboard" },
          targetType: "SYSTEM",
          description: "Emergency maintenance via dashboard",
        },
      });
    }
  ...
  private async sendMassNotification(
    parameters?: Record<string, any>,
    adminUserId?: string,
  ) {
  ...
  private async extendTrialPeriods(
    parameters?: Record<string, any>,
    adminUserId?: string,
  ) {
  ...
  private async processBulkRefunds(
    parameters?: Record<string, any>,
    adminUserId?: string,
  ) {
