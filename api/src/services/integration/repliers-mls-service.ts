...
export class RepiersMlsService {
  ...
  private setupInterceptors(): void {
    ...
    this.repliesClient.interceptors.response.use(
      (response: AxiosResponse) => {
        this.logger.debug(`Repliers API request succeeded: ${response.config.url}`);
        return response;
      },
      (error) => {
        this.logger.error(`Repliers API request failed: ${error.response?.status} ${error.response?.statusText}`);
  ...
  private async checkRateLimit(providerId: string): Promise<void> {
    ...
    if (limiter.requests >= limiter.limit) {
      ...
      this.logger.warn(`Rate limit reached for ${providerId}, waiting ${waitTime}ms`);
  ...
  async searchProperties(criteria: SearchCriteria): Promise<PropertyListing[]> {
    try {
      ...
      throw new Error(`Repliers search failed: ${error.message}`);
    }
  }
  ...
  async getPropertyDetails(listingId: string): Promise<PropertyListing | null> {
    try {
      ...
      this.logger.error(`Error fetching listing details for ${listingId}:`, error);
   ...
   */
  async addCustomMLSProvider(providerId: string, config: CustomMLSConfig): Promise<void> {
    try {
      ...
      this.logger.error(`Failed to add custom MLS provider ${providerId}:`, error);
      throw error;
    }
  }
    ...
    const customSearches = Array.from(this.customProviders.entries()).map(
      async ([providerId, provider]) => {
        try {
          ...
          this.logger.error(`Custom provider ${providerId} search failed:`, error);
  ...
  getProvidersStatus(): {
    ...
    custom: { [providerId: string]: { status: string; name: string; endpoint: string } };
  } {
    return {
      repliers: {
        status: 'active',
  ...
  private mapRepliersListing(listing: any): PropertyListing {
    return {
      ...
      coordinates: listing.coordinates || listing.location ? {
        lat: parseFloat(listing.coordinates?.lat || listing.location?.latitude),
        lng: parseFloat(listing.coordinates?.lng || listing.location?.longitude)
      } : undefined,
  ...
  async healthCheck(): Promise<{
    ...
    health.overall = health.repliers || Object.values(health.customProviders).some(h => h);
...
class CustomMLSProvider {
  ...
  async testConnection(): Promise<void> {
    try {
      ...
      throw new Error(`Custom MLS provider ${this.providerId} connection test failed: ${error.message}`);
    }
  }
  ...
  async searchProperties(criteria: SearchCriteria): Promise<PropertyListing[]> {
    try {
      ...
      throw new Error(`Custom provider ${this.providerId} search failed: ${error.message}`);
    }
  }
  ...
  private mapCustomListing(listing: any): PropertyListing {
    ...
    return {
      ...
      bathrooms: parseFloat(this.getFieldValue(listing, mapping.bathrooms)) || 0,
      ...
      daysOnMarket: this.calculateDaysOnMarket(this.getFieldValue(listing, mapping.listingDate)),
      status: this.getFieldValue(listing, mapping.status) || 'Active',
      photos: mapping.photos ? this.getFieldValue(listing, mapping.photos) || [] : [],
      coordinates: mapping.coordinates ? {
        lat: parseFloat(this.getFieldValue(listing, mapping.coordinates.lat)),
        lng: parseFloat(this.getFieldValue(listing, mapping.coordinates.lng))
      } : undefined,
