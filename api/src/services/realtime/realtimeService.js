...
import { createWebSocketServer, getWebSocketServer } from './websocketServer.js';
...
export class RealtimeService {
  ...
  async sendUserAlert(userId, alert) {
    ...
    try {
      await this.wsServer.broadcastAlert(userId, {
        ...
        message: alert.message || 'A new property matching your criteria is available',
   ...
   */
  async notifyPropertyChange(propertyId, changes, interestedUserIds = []) {
    if (!this.initialized) {
      console.warn('Real-time service not initialized, skipping property change notification');
      return;
    }

    try {
      await this.wsServer.broadcastPropertyChange(propertyId, {
      ...
      console.log(`🏠 Property change notification sent for ${propertyId} to ${interestedUserIds.length} users`);
    } catch (error) {
  ...
  async sendSystemNotification(notification) {
    if (!this.initialized) {
      console.warn('Real-time service not initialized, skipping system notification');
      return;
    }

    try {
      await this.redisClient.publish('system_notifications', JSON.stringify({
  ...
  async processNewListing(listing) {
    if (!this.initialized) {
      console.warn('Real-time service not initialized, skipping listing processing');
      return;
    }
    ...
    try {
      ...
      console.log(`🎯 Processed new listing ${listing.id}, matched ${matchedUsers.length} users`);
    } catch (error) {
  ...
  async findMatchingUsers(listing) {
  ...
  calculateAlertPriority(listing, userPreferences) {
    let priority = 'medium';
    
    if (listing.opportunityScore > 80) priority = 'high';
    if (listing.type === 'power_of_sale' || listing.type === 'tax_sale') priority = 'high';
    if (userPreferences?.priorityTypes?.includes(listing.type)) priority = 'high';
