...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      logger.info(
        `MLS search completed for user ${req.user?.id}: ${results.total || results.listings?.length} results`,
      );
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (!listing && provider && provider !== "repliers") {
        ...
        logger.info(
          `Custom provider search for listing ${id} not yet implemented`,
        );
      }
    ...
    try {
      ...
      if (!providerId || !name || !endpoint || !authentication || !mapping) {
        return res.status(400).json({
          success: false,
          message:
            "Missing required fields: providerId, name, endpoint, authentication, mapping",
        });
      }
      ...
      logger.info(
        `Admin ${req.user?.email} added custom MLS provider: ${providerId}`,
      );
...
router.delete(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      logger.info(
        `Admin ${req.user?.email} removed custom MLS provider: ${providerId}`,
      );
      ...
      logger.error(
        `Error removing custom MLS provider ${req.params.providerId}:`,
        error,
      );
      return res.status(500).json({
        success: false,
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const testProvider =
        new (require("../services/integration/repliers-mls-service").CustomMLSProvider)(
          "test",
          testConfig,
          new CacheManager(),
        );
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      return res.json({
        ...
        data: examples,
        message:
          "These are example configurations for common MLS provider types. Customize the mapping based on your specific API response structure.",
      });
    } catch (error) {
