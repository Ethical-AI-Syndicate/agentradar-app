...
import { CustomerOnboardingService, BrokerageClientData } from '../services/customerOnboardingService';
...
const workflowService = new AutomatedWorkflowService();
...
const brokerageClientSchema = Joi.object({
  ...
  subscriptionTier: Joi.string().valid('FREE', 'SOLO_AGENT', 'PROFESSIONAL', 'TEAM_ENTERPRISE', 'WHITE_LABEL').required()
});
 ...
 * @access Admin
 */
router.post('/clients', authenticateToken, requireAdmin, async (req: Request, res: Response) => {
  try {
 ...
 * @access Admin
 */
router.get('/clients', authenticateToken, requireAdmin, async (req: Request, res: Response) => {
  try {
 ...
 * @access Admin
 */
router.get('/clients/:id', authenticateToken, requireAdmin, async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    ...
    const onboardingStatus = await onboardingService.getOnboardingStatus(id);
 ...
 * @access Admin
 */
router.post('/steps/complete', authenticateToken, requireAdmin, async (req: Request, res: Response) => {
  try {
    ...
    await onboardingService.completeOnboardingStep(stepId, data);
 ...
 * @access Admin
 */
router.get('/metrics/success', authenticateToken, requireAdmin, async (req: Request, res: Response) => {
  try {
    ...
    const [
      ...
      (global as any).prisma.brokerageClient.count({ where: { isActive: true } }),
    ...
    const avgOnboardingTime = averageOnboardingTime.length > 0
      ? averageOnboardingTime.reduce((sum: number, metric: any) => sum + Number(metric.value), 0) / averageOnboardingTime.length
      : 0;
    
    const onboardingBreakdown = onboardingStats.reduce((acc: any, stat: any) => {
      acc[stat.onboardingStatus.toLowerCase()] = stat._count.onboardingStatus;
      return acc;
    }, {});
    
    const newClientsThisPeriod = await (global as any).prisma.brokerageClient.count({
 ...
 * @access Admin
 */
router.get('/health-scores', authenticateToken, requireAdmin, async (req: Request, res: Response) => {
  try {
    ...
    const healthScores = await (global as any).prisma.customerHealthScore.findMany({
      where,
      take: limit,
    ...
    // Risk level summary
    const riskSummary = await (global as any).prisma.customerHealthScore.groupBy({
      by: ['riskLevel'],
 ...
 * @access Admin
 */
router.post('/workflows/execute', authenticateToken, requireAdmin, async (req: Request, res: Response) => {
  try {
 ...
 * @access Admin
 */
router.post('/workflows/check', authenticateToken, requireAdmin, async (req: Request, res: Response) => {
  try {
    console.log('🔍 Running workflow checks...');
    
    await workflowService.checkAndExecuteWorkflows();
 ...
 * @access Admin
 */
router.get('/communications/:clientId', authenticateToken, requireAdmin, async (req: Request, res: Response) => {
  try {
    const { clientId } = req.params;
    console.log(`📧 Fetching communication history for client: ${clientId}`);
    
    const communications = await (global as any).prisma.customerCommunication.findMany({
