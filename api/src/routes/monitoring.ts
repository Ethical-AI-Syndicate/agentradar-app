...
router.get('/dashboard', authenticateToken, requireAdmin, async (req: Request, res: Response) => {
  try {
    ...
    const dashboardMetrics = await observabilityDashboard.getDashboardMetrics();
 ...
 * @access Admin Only
 */
router.get('/dashboard/html', authenticateToken, requireAdmin, async (req: Request, res: Response) => {
  try {
...
router.get('/performance', authenticateToken, requireAdmin, async (req: Request, res: Response) => {
  try {
    ...
    const performanceAnalytics = apmMonitor.generatePerformanceAnalytics(timeRange);
 ...
 * @access Admin Only
 */
router.get('/insights', authenticateToken, requireAdmin, async (req: Request, res: Response) => {
  try {
    ...
    res.status(200).json({
      ...
      currentMetrics: currentMetrics ? {
        ...
        criticalAlerts: currentMetrics.alerts.filter(a => a.severity === 'critical').length
      } : null,
      timestamp: new Date().toISOString()
    });
 ...
 * @access Admin Only
 */
router.get('/metrics/api', authenticateToken, requireAdmin, async (req: Request, res: Response) => {
  try {
 ...
 * @access Admin Only
 */
router.get('/export/performance', authenticateToken, requireAdmin, async (req: Request, res: Response) => {
  try {
 ...
 * @access Admin Only
 */
router.get('/export/observability', authenticateToken, requireAdmin, async (req: Request, res: Response) => {
  try {
    console.log('📤 Observability report export requested');
    
    const exportPath = await observabilityDashboard.exportObservabilityReport();
    ...
    res.status(200).json({
      ...
      monitoring: {
        ...
        statistics: {
          ...
          lastHealthCheck: currentMetrics?.timestamp || new Date().toISOString()
        }
      },
      timestamp: new Date().toISOString()
    });
