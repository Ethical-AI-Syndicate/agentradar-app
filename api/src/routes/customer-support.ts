...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const ticket = await customerSupportSystem.createTicket(
        ticketData,
        req.user?.id,
      );

      logger.info(
        `Support ticket created: ${ticket.id} for ${customerEmail || req.user?.email}`,
      );
...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const tickets = await customerSupportSystem.getTickets(filter);

      // Simple pagination for stubbed service
      ...
      return res.json({
        ...
        data: {
          ...
          pagination: {
            ...
            pages: Math.ceil(tickets.length / limitNum),
          },
        },
      });
    } catch (error) {
...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const ticket = await customerSupportSystem.getTicket(ticketId);
      ...
      logger.error(
        `Error fetching ticket details ${req.params.ticketId}:`,
        error,
      );
      return res.status(500).json({
        success: false,
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const response = await customerSupportSystem.addTicketResponse(ticketId, {
        content,
        authorId,
        isInternal,
        attachments,
      ...
      logger.info(
        `Response added to ticket ${ticketId} by user ${req.user?.email}`,
      );
      ...
      logger.error(
        `Error adding response to ticket ${req.params.ticketId}:`,
        error,
      );
      return res.status(500).json({
        success: false,
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      logger.warn(
        `Ticket ${ticketId} escalated by admin ${req.user?.email}: ${reason}`,
      );
...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const metrics = await customerSupportSystem.getSupportMetrics();
...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const agents = await customerSupportSystem.getAvailableAgents();
      const performance = { agents, dateRange }; // Stub data
...
router.get("/knowledge-base", async (req: Request, res: Response) => {
  try {
    ...
    const articles = await customerSupportSystem.getKnowledgeBaseArticles();
    const result = {
...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const responses = await customerSupportSystem.getCannedResponses();
...
router.get("/health", async (req: Request, res: Response) => {
  try {
    ...
    const tickets = await customerSupportSystem.getTickets({});
