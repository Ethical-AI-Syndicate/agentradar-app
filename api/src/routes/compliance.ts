 ...
 */
router.get('/gdpr/access-request', authenticateToken, async (req: Request, res: Response) => {
  try {
    const userId = (req as any).user.id;
    
    console.log(`📋 Processing GDPR access request for user: ${userId}`);
    
    const dataExport = await complianceService.handleDataSubjectAccessRequest(userId);
    ...
    res.json({
      ...
      compliance: {
        ...
        retention_notice: 'This data export contains all personal data we process about you'
      }
    });
  } catch (error) {
...
router.delete('/gdpr/delete-account', authenticateToken, async (req: Request, res: Response) => {
  try {
    ...
    const deleted = await complianceService.handleRightToBeForgotten(userId, reason);
    ...
    if (deleted) {
      res.json({
        ...
        compliance: {
          ...
          processed_at: new Date().toISOString(),
          notice: 'Your personal data has been anonymized and will be retained only for legal compliance purposes'
        }
      });
    } else {
      res.status(400).json({
 ...
 */
router.get('/gdpr/export-data', authenticateToken, async (req: Request, res: Response) => {
  try {
    const userId = (req as any).user.id;
    ...
    console.log(`📤 Processing GDPR data portability request for user: ${userId}, format: ${format}`);
    
    const exportData = await complianceService.handleDataPortabilityRequest(userId, format as 'json' | 'csv');
...
router.get('/sox/audit-report', authenticateToken, requireAdmin, complianceMiddleware.soxCompliance, async (req: Request, res: Response) => {
  try {
    ...
    console.log(`📊 Generating SOX audit report from ${startDate} to ${endDate}`);
    
    const auditReport = await complianceService.generateSOXAuditReport(
      new Date(startDate),
      new Date(endDate)
    );
 ...
 */
router.get('/real-estate/license-validation', authenticateToken, requireAdmin, complianceMiddleware.realEstateCompliance, async (req: Request, res: Response) => {
  try {
    console.log('🏠 Running real estate license compliance check');
    
    const validationReport = await complianceService.validateRealEstateLicenses();
    ...
    res.json({
      ...
      compliance: {
        ...
        next_check: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString() // 30 days
      }
    });
  } catch (error) {
 ...
 */
router.post('/data-retention/enforce', authenticateToken, requireAdmin, complianceMiddleware.dataRetentionCompliance, async (req: Request, res: Response) => {
  try {
    console.log('🧹 Enforcing data retention policies');
    
    await complianceService.enforceDataRetentionPolicies();
    ...
    res.json({
      ...
      compliance: {
        ...
        next_enforcement: new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString() // 7 days
      }
    });
  } catch (error) {
...
router.get('/health', authenticateToken, requireAdmin, async (req: Request, res: Response) => {
  try {
    const healthStatus = {
      gdpr: {
        status: 'ACTIVE',
        features: ['Access Requests', 'Right to be Forgotten', 'Data Portability'],
