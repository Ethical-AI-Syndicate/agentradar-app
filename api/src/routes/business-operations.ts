...
router.get("/dashboard", async (req: Request, res: Response) => {
  try {
    ...
    logger.info(
      `Business operations dashboard accessed by user ${req.user?.id}`,
    );
...
router.put("/users/:id/subscription", async (req: Request, res: Response) => {
  try {
    ...
    if (
      !subscriptionTier ||
      !Object.values(SubscriptionTier).includes(subscriptionTier)
    ) {
      return res.status(400).json({
        success: false,
    ...
    await businessOperations.changeUserSubscription(
      id,
      subscriptionTier,
      req.user!.id,
    );
    ...
    logger.info(
      `User subscription changed: ${id} -> ${subscriptionTier} by admin ${req.user?.email}`,
    );
...
router.post("/financial/operation", async (req: Request, res: Response) => {
  try {
    ...
    const result = await businessOperations.processFinancialOperation(
      operation,
      req.user!.id,
    );

    logger.info(
      `Financial operation processed: ${operation.type} for user ${operation.userId} by admin ${req.user?.email}`,
    );
...
router.post("/teams/:teamId/members", async (req: Request, res: Response) => {
  try {
    ...
    logger.info(
      `User assigned to team: ${userId} -> ${teamId} as ${role} by admin ${req.user?.email}`,
    );
...
router.delete(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      logger.info(
        `User removed from team: ${userId} -> ${teamId} by admin ${req.user?.email}`,
      );
