...
import { getRealtimeService } from "../services/realtime/realtimeService";
...
router.post(
  ...
  [
    ...
    body("type")
      .optional()
      .isIn(["test", "system", "property", "market"])
...
router.post(
  ...
  [
    ...
    body("interestedUsers")
      .optional()
      .isArray()
      .withMessage("Interested users must be an array"),
  ],
  async (req: Request, res: Response) => {
    try {
      const errors = validationResult(req);
      ...
      await realtimeService.notifyPropertyChange(
        propertyId,
        {
          ...changes,
          timestamp: new Date().toISOString(),
...
router.post(
  ...
  [
    ...
    body("listing.address")
      .notEmpty()
      .withMessage("Listing address is required"),
