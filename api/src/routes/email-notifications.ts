...
router.post("/admin/send", async (req: Request, res: Response) => {
  try {
    const { to, subject, html, text, categories, customArgs, sendAt } =
      req.body;
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      logger.info(
        `Ticket notification email sent to: ${userEmail} for ticket: ${ticketData.ticketNumber}`,
      );

      return res.json({
        success: result.success,
        message: result.success
          ? "Ticket notification sent successfully"
          : "Failed to send ticket notification",
        data: {
          messageId: result.messageId,
          error: result.error,
        },
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const result = await emailService.sendAlertNotificationEmail(
        userEmail,
        alertData,
      );

      logger.info(
        `Alert notification email sent to: ${userEmail} for alert: ${alertData.title}`,
      );
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      logger.info(
        `Subscription change email sent to: ${userEmail} - ${subscriptionData.oldTier} -> ${subscriptionData.newTier}`,
      );

      return res.json({
        success: result.success,
        message: result.success
          ? "Subscription change notification sent successfully"
